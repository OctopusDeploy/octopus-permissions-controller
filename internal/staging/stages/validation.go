...
func NewValidationStage(client client.Client, engine *rules.InMemoryEngine, targetNamespaces []string) *ValidationStage {
...
func (vs *ValidationStage) Execute(ctx context.Context, batch *staging.Batch) error {
	...
	if err := vs.checkResourceVersions(ctx, batch, result); err != nil {
...
func (vs *ValidationStage) checkResourceVersions(ctx context.Context, batch *staging.Batch, result *staging.ValidationResult) error {
	// For now, we'll skip resource version checks since we're doing full reconciliation
	// In the future, we could add resource version tracking to detect concurrent modifications
	return nil
}
...
func (vs *ValidationStage) getTargetNamespaces() []string {
	if vs.targetNamespaces != nil && len(vs.targetNamespaces) > 0 {
