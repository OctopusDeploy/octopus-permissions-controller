...
KIND_CLUSTER ?= octopus-permissions-controller-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name octopus-permissions-controller-builder
	$(CONTAINER_TOOL) buildx use octopus-permissions-controller-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --build-arg VERSION=$(VERSION) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm octopus-permissions-controller-builder
	rm Dockerfile.cross
...
.PHONY: deploy-tests
deploy-tests: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
	$(KUSTOMIZE) build config/default-test | $(KUBECTL) apply -f -
...
HELM_NAMESPACE ?= octopus-permissions-controller-system
## Name of the Helm release
HELM_RELEASE ?= octopus-permissions-controller
